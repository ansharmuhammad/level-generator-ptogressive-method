...
func rightComponent(graph: Graph, vertex: Vertex, edge: Edge):
	...
	var vertex5 = graph.addVertex("", VERTEX_TYPE.KEY)
	var vertex6 = graph.addVertex("", VERTEX_TYPE.TASK)
	
	edge1.target = vertex3.name
	graph.connectVertex(vertex3, vertex4)
	graph.connectVertex(vertex1, vertex6)
	graph.connectVertex(vertex6, vertex5)
	graph.connectVertex(vertex5, vertex3)
	
	graph.connectVertex(vertex4, vertex2)
	
	graph.connectVertex(vertex5, vertex4, EDGE_TYPE.KEY_LOCK)
	
	print("execute rule KL2 at" + str(vertex) + " " + str(edge))
