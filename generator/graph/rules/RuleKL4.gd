extends IRule

## goal lock
class_name RuleKL4

# condition
func leftComponent(graph: Graph, vertex: Vertex, edge: Edge) -> bool:
	if edge == null:
		return false
	if edge.target == "" or edge.target == null:
		return false
	
	var vertex2 = graph.getVertexByName(edge.target)
	if vertex2.type == VERTEX_TYPE.GOAL:
		return true
	
	return false

# replace graph component
func rightComponent(graph: Graph, vertex: Vertex, edge: Edge):
	var vertex1 = vertex
	var edge1 = edge
	
	var vertex2 = graph.getVertexByName(edge.target)
	var vertex3 = graph.addVertex("", VERTEX_TYPE.KEY)
	var vertex4 = graph.addVertex("", VERTEX_TYPE.TASK)
	var vertex5 = graph.addVertex("", VERTEX_TYPE.LOCK)
	
	edge1.target = vertex3.name
	graph.connectVertex(vertex3, vertex4)
	graph.connectVertex(vertex4, vertex5)
	graph.connectVertex(vertex5, vertex2)
	
	graph.connectVertex(vertex3, vertex5, EDGE_TYPE.KEY_LOCK)
	
	print("execute rule KL4 at" + str(vertex) + " " + str(edge))
